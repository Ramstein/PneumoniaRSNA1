...
class LearnNmsOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        nms_attention_1 = nms_attention_nd(nms_embedding_feat,
                                           ...
                                           nms_query_1_weight, nms_query_1_bias, nms_key_1_weight, nms_key_1_bias,
                                           nms_linear_out_1_weight, nms_linear_out_1_bias, num_rois=self.first_n,
                                           index=1,
        ...
        if num_valid_classes == self.num_fg_classes:
            ...
            full_nms_conditional_score = nd.concat(nms_conditional_score, nd.zeros((self.first_n,
                                                                                    self.num_fg_classes - num_valid_classes,
                                                                                    self.num_thresh),
        ...
        full_nms_conditional_score = full_nms_conditional_score.transpose((1, 0, 2)).take(restore_indexes).transpose(
            (1, 0, 2))
        ...
        if globals()['learn_nms_count'] % 250 == 0:
            print("--->> learn nms running average time cost: {}".format(float(sum(globals()['learn_nms_time'])) / (
                1000 if globals()['learn_nms_count'] > 1000 else globals()['learn_nms_count'])))
...
class LearnNmsProp(mx.operator.CustomOpProp):
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return LearnNmsOperator(self.num_fg_classes, self.bbox_means, self.bbox_stds, self.first_n,
                                self.class_agnostic, self.num_thresh, self.class_thresh, self.nongt_dim,
                                self.has_non_gt_index)
