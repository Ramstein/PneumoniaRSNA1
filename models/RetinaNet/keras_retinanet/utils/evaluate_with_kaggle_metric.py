...
def KaggleEvaluate(model_save_path,
    ...
    for index, each_img in enumerate(np.unique(folds_df.filepath)):
        ...
        if tmp_df.class_y.iloc[0] == 0:
            ...
            gt_bboxes = [np.asarray((row.x1, row.y1, row.x2 - row.x1, row.y2 - row.y1)) for rownum, row in
                         tmp_df.iterrows()]
    ...
    results_df_positives_only[["mAP", "threshold"]].groupby("threshold").mean().reset_index().to_csv(
        "{}-positives-only.csv".format(results_save_path.replace(".csv", "")), index=False)
    #
