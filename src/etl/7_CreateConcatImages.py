...
def createConcatImages(folds_df,
    ...
    pos_images = [img for img in images if
                  img.split("/")[-1].split(".")[0] in list(labels_df[labels_df["class"] == "Lung Opacity"].patientId)]
    neg_images = [img for img in images if
                  img.split("/")[-1].split(".")[0] in list(labels_df[labels_df["class"] != "Lung Opacity"].patientId)]
    pos_images_by_fold = []
    for each_fold in range(10):
        pos_images_by_fold.append([img for img in pos_images if img.split("/")[-1].split(".")[0] in list(
            labels_df[labels_df.fold == each_fold].patientId)])
    # TRAIN IMAGES #
